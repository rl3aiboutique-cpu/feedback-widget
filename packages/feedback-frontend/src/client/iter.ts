...
export async function getIterVersion(
  ...
  const url = `${_base(bindings)}${_prefix(bindings)}/iterate/sessions/${sessionId}/iterations/${versionId}`;
  const resp = await fetch(url, {
    credentials: "include",
    headers: await _headers(bindings),
  });
...
export async function getMyIterUsage(bindings: FeedbackHostBindings): Promise<IterUsageRead> {
...
function _parseSseFrame(frame: string): IterStreamEvent | null {
  ...
  try {
    ...
    return {
      type: event as IterStreamEvent["type"],
      ...parsed,
    } as IterStreamEvent;
  } catch {
    return null;
