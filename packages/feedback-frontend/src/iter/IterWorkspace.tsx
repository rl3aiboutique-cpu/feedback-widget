...
export default function IterWorkspace({
	sessionId,
	onClose,
}: IterWorkspaceProps) {
	...
	const renderedMarkdown = isStreaming
		? ""
	...
	const latestVersionAt = latestVersion
		? Date.parse(latestVersion.created_at)
		: 0;
	...
	const progress =
		totalAssumptions > 0
			? Math.round((resolvedAssumptions / totalAssumptions) * 100)
			: 0;
	...
	const [tab, setTab] = useState<"document" | "assumptions" | "activity">(
		"document",
	);
	const [seenAssumptionsBadge, setSeenAssumptionsBadge] = useState(false);
	useEffect(() => {
		if (
			!seenAssumptionsBadge &&
			openAssumptionCount > 0 &&
			tab === "document"
		) {
...
function TabBar(props: {
	...
	return (
		<div
			className="flex items-center gap-1 border-b bg-card px-3"
			role="tablist"
		>
			{tabs.map((t) => {
	...
	return (
		<div className="mx-auto flex w-full max-w-[1400px] flex-row gap-6 p-4 lg:p-6">
			{props.markdown && !props.streaming && (
				<DocumentTOC markdown={props.markdown} />
			)}
			<main className="flex flex-1 flex-col">
				...
				{props.streamStatus === "error" && (
					<ErrorBanner
						code={props.streamErrorCode}
						message={props.streamErrorMessage}
					/>
				)}
			</main>
		</div>
				...
				<ul className="space-y-1 text-sm">
					{headings.map((h) => (
						<li
							key={h.id}
							className={
								h.level === 2 ? "ml-3 text-xs text-muted-foreground" : ""
							}
						>
							<a
								href={`#${h.id}`}
			...
			<footer className="border-t bg-card px-4 py-3 text-sm">
				<div className="flex items-center gap-3">
					...
					{props.package?.presigned_zip_url && (
						<a
							href={props.package.presigned_zip_url}
		...
		<footer className="space-y-2 border-t bg-card px-4 py-3">
			{props.needsReviewIteration && (
				<div className="rounded-md border border-emerald-300 bg-emerald-50 px-3 py-2 text-xs text-emerald-900 dark:border-emerald-800 dark:bg-emerald-900/20 dark:text-emerald-200">
					...
					<p>
						...
						<strong>Run iteration</strong> so the AI rewrites the working
						document with your answers baked in. Once you review that version,
						you can finalize.
					</p>
				</div>
			)}
			...
			{props.openAssumptions > 0 && status !== "draft" && (
				<p className="text-xs text-amber-700">
					Resolve all {props.openAssumptions} open assumption(s) before
					iterating.
				</p>
			)}
			{showFinalizeWarning && (
	...
	return (
		...
		>
			...
			<p className="text-sm text-muted-foreground">
				The dev team interprets the package as the source of truth. If you
				finalize now, the following items aren't fully clarified:
			</p>
			<ul className="mt-3 list-disc space-y-1 pl-5 text-sm">
				{props.openAssumptions > 0 && (
					<li>
						<strong>{props.openAssumptions}</strong> assumption
						{props.openAssumptions === 1 ? " is" : "s are"} still open and will
						be packaged as <em>unresolved</em>.
					</li>
				)}
				{props.needsReviewIteration && (
					<li>
						You haven't run a review iteration since resolving assumptions, so
						the working document doesn't reflect your answers yet.
					</li>
				)}
				{!props.hasAnyVersion && (
			...
			<p className="mt-3 text-xs text-muted-foreground">
				You can still finalize — useful when you'd rather hand off a partial
				spec than block on perfection. The dev team can ask follow-ups.
			</p>
...
const _KIND_BAND: Record<
	IterAssumptionRead["kind"],
	...
	if (!props.items.length) {
		return (
			<p className="text-xs text-muted-foreground">
				No assumptions on the current version yet.
			</p>
		);
	}
	...
	return (
		<div className="space-y-4">
			{_KIND_ORDER.map((kind) => {
				...
				const dotColor = (meta.band.split(" ")[0] ?? "").replace(
					"border-l-",
					"bg-",
				);
				return (
					<section key={kind}>
						...
						<div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
							{items.map((a) => (
								<div
									key={a.id}
									className={`rounded-md border-l-4 ${meta.band} [&>div]:border-l-0`}
								>
									<AssumptionCard
										assumption={a}
										disabled={props.disabled}
...
function FirstRunCard({ busy, onRun }: { busy: boolean; onRun: () => void }) {
	return (
		<div className="mb-3 rounded-md border bg-muted/40 p-4 text-sm">
			<p className="mb-2">
				The AI will read your feedback, attachments, and technical metadata,
				then propose user personas, user stories, a spec, and a list of every
				assumption it had to make. You will resolve those assumptions before
				iterating again.
			</p>
	...
	return (
		<div className="flex-1 space-y-5 overflow-auto rounded-lg border bg-card p-6 text-sm">
			...
			{_SECTION_SKELETONS.map((s, idx) => {
				...
				const isPast =
					activeSection &&
					_SECTION_SKELETONS.findIndex((x) => x.key === activeSection) > idx;
				...
				const titleClass = isActive
					...
					: isPast
						? "text-emerald-700"
						: "";
				const barBaseClass = isActive
					? "animate-pulse bg-primary/30"
					: "bg-muted";
				return (
					<div
						key={s.key}
						className={`rounded-md border p-4 transition-colors ${cardClass}`}
					>
						<div className="flex items-center gap-2">
							...
							{isActive && (
								<span className="text-xs text-primary">writing now…</span>
							)}
	...
	if (!props.markdown) {
		return (
			<div className="flex-1 rounded border bg-card p-6 text-center text-sm text-muted-foreground">
				Press &ldquo;Generate first version&rdquo; or run an iteration to
				populate the working document.
			</div>
		);
	}
		...
		void import("markdown-it").then(({ default: MarkdownIt }) => {
			...
			md.renderer.rules.heading_open = (tokens, idx, options, env, self) => {
				...
				const text = inline?.children
					? inline.children.map((c) => c.content).join("")
					: "";
				const id = `iter-toc-${text
					.toLowerCase()
					.replace(/[^a-z0-9]+/g, "-")
