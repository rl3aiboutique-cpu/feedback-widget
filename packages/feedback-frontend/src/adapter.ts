...
export function useDeleteFeedbackMutation(): UseMutationResult<
  void,
  Error,
  string
> {
  ...
  return useMutation({
    mutationFn: (id) =>
      _deleteJson(bindings, `/${encodeURIComponent(id)}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["feedback"] })
    },
  })
