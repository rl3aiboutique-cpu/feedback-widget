...
export function installBreadcrumbs(capacity: number = DEFAULT_CAPACITY): void {
  ...
  const onSubmit = (e: SubmitEvent) => {
    ...
    const ident = form.id || form.getAttribute("name") || form.getAttribute("action") || "<form>";
    _push({
      kind: "submit",
  ...
  recordRoute(window.location.pathname + window.location.search + window.location.hash);
